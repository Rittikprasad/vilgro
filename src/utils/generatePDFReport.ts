...
/**
 * Generate PDF report with personal information and graph visualization
 * Similar to the report shown in the image with company name, personal info, and graph
 */
export const generatePDFReport = (spoData: AdminSpoEntry): void => {
  const doc = new jsPDF();
  ...
  const companyName = spoData.organization?.name || 'Sample Company Pvt Ltd';
  ...
  const personalInfo = {
    name: [spoData.first_name, spoData.last_name].filter(Boolean).join(' ') || 'John Doe',
    email: spoData.email || 'john.doe@example.com',
    mobile: spoData.phone || '+91 9876454314',
    diitNo: spoData.organization?.cin_number || 'NT36756543',
  };
  ...
  const infoItems = [
    ...
    { label: 'Mobile:', value: personalInfo.mobile },
    { label: 'DIIT No:', value: personalInfo.diitNo },
  ];
  ...
  const impactScore = spoData.scores?.sections?.IMPACT ?? 0;
  const riskScore = spoData.scores?.sections?.RISK ?? 0;
  const returnScore = spoData.scores?.sections?.RETURN ?? 0;
